...
# organised in code.
class HttpClientTest < ServiceTest
  include Screenings

  def setup
    @http   = FakeHttp.new
    @client = Showtimes::Sensacine.new
  end

  def fetch_a_day(client = @client, http = @http)
    http.while_intercepting { client.sessions_for(cinema("Ocimax", sensacine_id: "E0628"), "2026-08-28") }
  end
  ...
  def test_a_failure_makes_the_client_back_off_much_harder
    ...
    fetch_a_day(Showtimes::Sensacine.new, healthy_provider)
    ...
    fetch_a_day(Showtimes::Sensacine.new, flaky_provider)
